...
export default function MobileDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await api.get('/dashboard/mobile-home');
        if (res.data.success) {
  ...
  const d = data || {
    rentPending: { amount: 0, tenantCount: 0 },
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="bg-white px-5 py-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        ...
        <div className="h-11 w-11 rounded-full bg-blue-100 flex items-center justify-center border-2 border-blue-500 overflow-hidden">

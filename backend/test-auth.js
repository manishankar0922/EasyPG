...
async function test() {
  ...
  const token = jwt.sign({ userId: user.id, role: user.role, organisationId: user.organisationId, branchId: user.branchId }, process.env.JWT_SECRET || 'easypg-super-secret-key-123-production-ready', { expiresIn: '7d' });
